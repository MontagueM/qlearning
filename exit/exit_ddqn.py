...
class DDQNExitAgent(nn.Module, Agent):
    def __init__(self, device, num_actions):
        super(DDQNExitAgent, self).__init__()
        self.device = device
    ...
    def get_state(self, game: ExitGame) -> State:
        return State(game.exit_position, self.get_grid(game))
    ...
    def get_reward(self, state: State, next_state: State, game: ExitGame) -> float:
        if state.complete:
            reward = 1000 / self.current_game_steps
        else:
            reward = -1
    ...
    def train(self) -> float:
        ...
        complete = torch.tensor([exp.state.complete for exp in batch])
        ...
        # 1 if not complete, 0 if complete
        complete_mask = complete.logical_not().float()
        ...
        y[r, actions] = rewards + self.discount_factor * q * complete_mask
